...
test.describe('Household lifecycle P1-9 (#110)', () => {
  ...
  test('leaver loses inventory access after confirming (0 reads)', async ({
    ...
    // The former household is no longer readable: its name appears nowhere
    // (header in error/empty state, or brand with another name).
    await expect(page.getByText(householdName, { exact: true })).toHaveCount(0, { timeout: 15000 });
  });
