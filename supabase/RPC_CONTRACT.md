...
| `consume_household_invitation` | `p_token text` | `uuid` | Locks and consumes one valid, unexpired, unrevoked token, creates a `member` membership, and returns the household ID atomically. Fails without consuming the token if the caller already belongs to any household, or if the household already has 2 members (PRD §4.2 cap 2, §8 P0-6: `23505 'household is full'`; invalid/expired/revoked/unknown tokens stay `22023`, lockout stays `P0001`). |
